...
class MSVR():
    ...
    # Add get_params and set_params for compatibility with Scikit-learn
...
def msvr_model(X, Y, cv=5, n_iter=20):
    """
    Multi-Output Support Vector Regression with RandomizedSearchCV.
    Args:
        X (array): Features.
        Y (array): Targets.
    ...
    Returns:
        metrics (dict): Cross-validated MAE, RMSE, and R².
    ...
    param_distributions = {
        ...
        'gamma': [0.01, 0.1, 1, 10]  # Only numeric values allowed
    }

    # RandomizedSearchCV
    random_search = RandomizedSearchCV(msvr, param_distributions, n_iter=n_iter,
                                       scoring='neg_mean_squared_error', cv=cv, random_state=42)
    random_search.fit(X, Y)

    # Best model and cross-validated predictions
    best_msvr = random_search.best_estimator_
    Y_pred = cross_val_predict(best_msvr, X, Y, cv=cv)
